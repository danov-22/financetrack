...
window.reviewFeedback = async (id, status) => { const reply = document.getElementById(`reply-${id}`)?.value.trim() || ""; if (status === "resolved" && !reply && !confirm("Resolve this feedback without sending the user a reply?")) return; try { await betaApi("/api/feedback", { method:"POST", body:JSON.stringify({ action:"admin-review", id, status, reply }) }); await loadBeta(); statusEl.textContent = reply ? "Feedback updated and the reply was added to the user’s inbox." : "Feedback checklist updated."; } catch (error) { statusEl.textContent = error.message; } };
